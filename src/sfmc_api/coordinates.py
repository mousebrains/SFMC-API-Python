...
Why Slocum gliders use DDDMM format
------------------------------------

Most scientific software uses decimal degrees (e.g. 44.6253) for
coordinates, but Slocum gliders use a packed degrees-and-decimal-
minutes format called DDDMM.MMMM.  In this format the integer part
encodes degrees and the fractional part encodes minutes:

- Latitude ``4437.518`` means 44 degrees, 37.518 minutes north.
- Longitude ``-12406.120`` means 124 degrees, 06.120 minutes west
  (the minus sign indicates west).

This format dates back to early GPS receivers and marine navigation
conventions.  The Slocum firmware inherited it and uses it
everywhere: in dialog output, sensor values (``c_wpt_lat``,
``c_wpt_lon``), and mission-argument files (``.ma`` files).

Because virtually all oceanographic analysis tools expect decimal
degrees, you will need to convert back and forth.  That is what
this module provides.

Quick reference::

    # DDMM -> decimal
    >>> dddmm_to_decimal(3310.021)   #  33 deg 10.021'
    33.167016666...

    >>> dddmm_to_decimal(-11741.800)  # -117 deg 41.800'
    -117.696666...
